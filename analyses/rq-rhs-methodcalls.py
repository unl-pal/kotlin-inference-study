...
df_kotlin = df_kotlin[['project', 'maybe_local']].groupby('maybelocal').count() \
...
df_kotlin['formatted'] = df_kotlin.apply(lambda x: "{:,} ({:,.2f}%)".format(x.project, x.percent), axis=1)

df_java = get_deduped_df('method-calls-maybe-local', 'java', header='infer')
df_java = df_java[['project', 'maybe_local']].groupby('maybelocal').count() \
                .rename(index={False: 'Not Local', True: 'Possibly File-Local'})
java_total_calls = df_java.project.sum()
df_java['percent'] = df_java.project.div(java_total_calls).mul(100)
df_java['formatted'] = df_java.apply(lambda x: "{:,} ({:,.2f}%)".format(x.project, x.percent), axis=1)
