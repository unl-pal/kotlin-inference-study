...
df_size = pd.pivot_table(get_df('project-size', 'java', header='infer'),
                         index='project', columns='count_type', values='count')

df_stars = get_df('stars', 'java', header='infer')
df_num_devs = get_df('developer-count', 'java', header='infer')

df_usage = get_deduped_df('basic-usage', 'java', header='infer')
...
for factor in ['files', 'statements', 'stars', 'developers']:
    ...
    save_figure(fig, f'correlation-{factor}.png', 6, 6, 'java')
